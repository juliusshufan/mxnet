...
from mxnet.metric import Accuracy, TopKAccuracy, CrossEntropy, NegativeLogLikelihood, CompositeEvalMetric
import numpy as np
...
dataset_classes = {'mnist': 10, 'cifar10': 10, 'imagenet': 1000, 'dummy': 1000, 'sampleimgnet': 200}
...
def get_data_iters(dataset, batch_size, num_workers=1, rank=0):
    ...
    if dataset == 'mnist':
        ...
        train_data, val_data = get_cifar10_iterator(batch_size, (3, 32, 32),
                                                    num_parts=num_workers, part_index=rank)
    elif dataset == 'sampleimgnet':
        train_data, val_data = get_sampleimgnet_iterator(opt.data_dir, batch_size, (3, 224, 224),
                                                    num_parts=num_workers, part_index=rank)
        ...
        for i, batch in enumerate(train_data):
            ...
            if opt.log_interval and not (i+1)%opt.log_interval:
                ...
                logger.info('Epoch[%d] Batch [%d]\tSpeed: %f samples/sec\t%s=%f, %s=%f, %s=%f'%(
                               epoch, i, batch_size/(time.time()-btic), name[0], acc[0], name[1], acc[1], name[2], acc[2]))
            btic = time.time()
